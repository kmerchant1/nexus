...
from nexus.api.dependencies import get_db
...
async def _handle_pull_request(event: PullRequestEvent, db: AsyncSession) -> dict:
    ...
    logger.info(
        "Queued pr_update job for %s PR #%d",
        event.repository.full_name,
        event.number,
    )
...
async def github_webhook(
    ...
    if x_github_event == "installation":
        ...
        if event.action == "created":
            return await _handle_installation_created(event, db)
    ...
    if x_github_event == "pull_request":
        ...
        return await _handle_pull_request(event, db)
